...
fun DayApp(day: Day, scope: CoroutineScope) {
  ...
  val elf by day.currentElf.collectAsState()
    ...
    Column(
      ...
      Card {
        Column(modifier = Modifier.fillMaxWidth().padding(8.dp)) {
          Text(
            text = "Total for top elves: ${day.topTotal.value}",
...
@Composable
private fun TopElves(elves: List<Elf>) {
  Card {
    Card(modifier = Modifier.fillMaxWidth()) {
      Column(modifier = Modifier.fillMaxWidth().padding(8.dp)) {
        ...
        elves.forEach { elf ->
          Text(
            text = "${elf.total} = ${elf.snacks.joinToString(" + ")}",
          )
        }
      }
    }
...
fun ElfList(
  ...
  LazyColumn(
    ...
    items(elves.size) { index ->
      val elf = elves[index]
      ElfRow(elf, index == elfCursor, elf in topElves)
    }
  }
}

@Composable
fun ElfRow(elf: Elf, isIndex: Boolean, isTopElf: Boolean) {
  Row(
    modifier = Modifier
      .fillMaxWidth()
      .background(
        when {
          isIndex -> Color.Yellow
          isTopElf -> Color.Magenta
          else -> Color.White
        }
      )
  ) {
    Text(
      text = "${elf.total} = ${elf.snacks.joinToString(" + ")}",
      style = TextStyle(
        fontWeight = if (isTopElf) FontWeight.Bold else FontWeight.Normal
      )
    )
  }
}
